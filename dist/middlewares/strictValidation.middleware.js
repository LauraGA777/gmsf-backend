...
exports.sanitizeEmails = exports.sanitizePhoneNumbers = exports.strictLengthValidation = void 0;
const apiResponse_1 = __importDefault(require("../utils/apiResponse"));
// Middleware para validaciones estrictas de longitud
const strictLengthValidation = (req, res, next) => {
    try {
        const { body } = req;
        // Validaciones estrictas para campos comunes
        if (body.correo && typeof body.correo === 'string') {
            if (body.correo.length > 254) {
                return apiResponse_1.default.error(res, 'El correo electrónico no puede exceder 254 caracteres', 400);
            }
        }
        if (body.telefono && typeof body.telefono === 'string') {
            if (body.telefono.length > 20) {
                return apiResponse_1.default.error(res, 'El teléfono no puede exceder 20 caracteres', 400);
            }
            // Validar que solo contenga números
            if (!/^\d+$/.test(body.telefono)) {
                return apiResponse_1.default.error(res, 'El teléfono solo puede contener números', 400);
            }
        }
        if (body.numero_documento && typeof body.numero_documento === 'string') {
            if (body.numero_documento.length > 20) {
                return apiResponse_1.default.error(res, 'El número de documento no puede exceder 20 caracteres', 400);
            }
            if (body.numero_documento.length < 5) {
                return apiResponse_1.default.error(res, 'El número de documento debe tener mínimo 5 caracteres', 400);
            }
        }
        if (body.nombre && typeof body.nombre === 'string') {
            if (body.nombre.length > 100) {
                return apiResponse_1.default.error(res, 'El nombre no puede exceder 100 caracteres', 400);
            }
        }
        if (body.apellido && typeof body.apellido === 'string') {
            if (body.apellido.length > 100) {
                return apiResponse_1.default.error(res, 'El apellido no puede exceder 100 caracteres', 400);
            }
        }
        ...
        if (body.usuario && typeof body.usuario === 'object') {
            ...
            if (usuario.correo && typeof usuario.correo === 'string' && usuario.correo.length > 254) {
                return apiResponse_1.default.error(res, 'El correo del usuario no puede exceder 254 caracteres', 400);
            }
            if (usuario.telefono && typeof usuario.telefono === 'string' && usuario.telefono.length > 20) {
                return apiResponse_1.default.error(res, 'El teléfono del usuario no puede exceder 20 caracteres', 400);
            }
            if (usuario.numero_documento && typeof usuario.numero_documento === 'string') {
                if (usuario.numero_documento.length > 20) {
                    return apiResponse_1.default.error(res, 'El número de documento del usuario no puede exceder 20 caracteres', 400);
                }
                if (usuario.numero_documento.length < 5) {
                    return apiResponse_1.default.error(res, 'El número de documento del usuario debe tener mínimo 5 caracteres', 400);
                }
            }
            if (usuario.nombre && typeof usuario.nombre === 'string' && usuario.nombre.length > 100) {
                return apiResponse_1.default.error(res, 'El nombre del usuario no puede exceder 100 caracteres', 400);
            }
            if (usuario.apellido && typeof usuario.apellido === 'string' && usuario.apellido.length > 100) {
                return apiResponse_1.default.error(res, 'El apellido del usuario no puede exceder 100 caracteres', 400);
            }
        }
            ...
            for (let i = 0; i < body.beneficiarios.length; i++) {
                ...
                if (beneficiario.usuario && typeof beneficiario.usuario === 'object') {
                    ...
                    if (usuarioBenef.correo && typeof usuarioBenef.correo === 'string' && usuarioBenef.correo.length > 254) {
                        return apiResponse_1.default.error(res, `El correo del beneficiario ${i + 1} no puede exceder 254 caracteres`, 400);
                    }
                    if (usuarioBenef.telefono && typeof usuarioBenef.telefono === 'string' && usuarioBenef.telefono.length > 20) {
                        return apiResponse_1.default.error(res, `El teléfono del beneficiario ${i + 1} no puede exceder 20 caracteres`, 400);
                    }
                    if (usuarioBenef.numero_documento && typeof usuarioBenef.numero_documento === 'string') {
                        if (usuarioBenef.numero_documento.length > 20) {
                            return apiResponse_1.default.error(res, `El número de documento del beneficiario ${i + 1} no puede exceder 20 caracteres`, 400);
                        }
                        if (usuarioBenef.numero_documento.length < 5) {
                            return apiResponse_1.default.error(res, `El número de documento del beneficiario ${i + 1} debe tener mínimo 5 caracteres`, 400);
                        }
                    }
                }
            }
            ...
            for (let i = 0; i < body.contactos_emergencia.length; i++) {
                ...
                if (contacto.telefono_contacto && typeof contacto.telefono_contacto === 'string') {
                    if (contacto.telefono_contacto.length > 20) {
                        return apiResponse_1.default.error(res, `El teléfono del contacto de emergencia ${i + 1} no puede exceder 20 caracteres`, 400);
                    }
                    if (!/^\d+$/.test(contacto.telefono_contacto)) {
                        return apiResponse_1.default.error(res, `El teléfono del contacto de emergencia ${i + 1} solo puede contener números`, 400);
                    }
                }
                if (contacto.nombre_contacto && typeof contacto.nombre_contacto === 'string' && contacto.nombre_contacto.length > 100) {
                    return apiResponse_1.default.error(res, `El nombre del contacto de emergencia ${i + 1} no puede exceder 100 caracteres`, 400);
                }
            }
        }
        next();
        ...
        return apiResponse_1.default.error(res, 'Error interno del servidor en validación', 500);
    }
};
exports.strictLengthValidation = strictLengthValidation;
// Middleware específico para filtrar caracteres no numéricos en teléfonos
const sanitizePhoneNumbers = (req, res, next) => {
    try {
        const { body } = req;
        ...
        if (body.beneficiarios && Array.isArray(body.beneficiarios)) {
            body.beneficiarios.forEach((beneficiario) => {
        ...
        if (body.contactos_emergencia && Array.isArray(body.contactos_emergencia)) {
            body.contactos_emergencia.forEach((contacto) => {
        ...
        return apiResponse_1.default.error(res, 'Error interno del servidor en sanitización', 500);
    }
};
exports.sanitizePhoneNumbers = sanitizePhoneNumbers;
// Middleware para normalizar correos electrónicos
const sanitizeEmails = (req, res, next) => {
    try {
        const { body } = req;
...
exports.sanitizeEmails = sanitizeEmails;
exports.default = {
    strictLengthValidation: exports.strictLengthValidation,
    sanitizePhoneNumbers: exports.sanitizePhoneNumbers,
    sanitizeEmails: exports.sanitizeEmails
};
