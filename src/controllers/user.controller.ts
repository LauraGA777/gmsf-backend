...
export class UserController {
    ...
    public async checkDocumentExists(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            const result = await this.userService.checkDocumentExists(numero_documento, excludeUserId as string | undefined);
            ApiResponse.success(res, result, result.exists ? 'Número de documento ya existe' : 'Número de documento disponible');
        } catch (error) {
            next(error);
        }
    }
